...
namespace Southxchange.Nxt
{
    ...
    ///   database or other unexpected problems.
    ...
    class NxtConnector : IConnector
    {
        ...
        public List<Transaction> ListTransactions()
        {
            ...
            TransferFundsToMainAccount(transactions);
        ...
        private void TransferFundsToMainAccount(List<Transaction> transactions)
        {
            var accountService = serviceFactory.CreateAccountService();
            ...
            foreach (var transaction in transactionsByAddress.Where(kvp => kvp.Value > Fee.Nxt))
            {
                ...
                var amount = NxtLib.Amount.CreateAmountFromNxt(transaction.Value - parameters.Fee.Nxt);
                var internalTransaction = accountService.SendMoney(parameters, mainAccount.Address, amount).Result;
            }
        }
